...
Each of those expression types supports by-ref parameters and write-backs to `Index`, `MemberAccess`, `ArrayIndex`, and `Call` (for `ArrayAccess` via the `Array.Get` method) nodes.
